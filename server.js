...
if (process.env.EXTERNAL_IP) {
  ...
  console.warn(
    "No external IP configured - TURN server may not work correctly from outside the network"
  );
}
...
try {
  ...
  console.log(
    `Listening on: ${config.listeningIps.join(", ")}:${config.listeningPort}`
  );
  console.log(
    `External IP: ${
      config.externalIps ? config.externalIps[0] : "Not configured"
    }`
  );
...
process.on("SIGINT", () => {
  console.log("Received SIGINT. Shutting down TURN server...");
  try {
    server.stop();
    process.exit(0);
  } catch (error) {
